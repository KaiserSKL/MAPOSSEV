...
if (! file_exists($settings_file)) {
    exit('¡Archivo de configuración no encontrado!');
} else {
...
if (! empty($_POST)) {
    ...
    if (! ($host && $dbuser && $dbname && $full_name && $email && $login_password)) {
        echo json_encode(['success' => false, 'message' => 'Por favor, rellene todos los campos.']);
        exit();
    }
    ...
    if ($is_installed) {
        echo json_encode(['success' => false, 'message' => '¡Parece que esta aplicación ya está instalada! No puedes reinstalarla.']);
        exit();
    }
    ...
    if (file_put_contents('..' . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . '.env', $env_file)) {
        ...
        echo json_encode(['success' => false, 'message' => 'Error al crear el archivo env.']);
    }
